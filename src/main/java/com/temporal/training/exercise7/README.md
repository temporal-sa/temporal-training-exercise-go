...
Configure the activity stub to disable automatic retries by setting `maxAttempts = 1` in the retry options.

### 2. Manual Retry Logic
Implement the `executeWithRetry` method that:
- Executes operations in a loop
- Catches exceptions and sets status to `RETRYING`
